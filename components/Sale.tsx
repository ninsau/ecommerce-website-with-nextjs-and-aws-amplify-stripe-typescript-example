                    ...
                    {sales.map((sale) => (
                      <tr key={sale.id}>
                        ...
                        <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                          {truncate(sale.content)}
                        </td>
                        <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                          {truncate(sale.buttonText)}
                        </td>

                        <td className="relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6">
                          <span
                            onClick={() => toggleModal(sale)}
                            className="text-green-600 cursor hover:text-green-900"
                          >
                            Edit<span className="sr-only"> {sale.title}</span>
                          </span>
                        </td>
                      </tr>
                    ))}
                  ...
                  <div>
                    <Formik
                      ...
                      onSubmit={async (values) => {
                        ...
                        const original = await DataStore.query(Sale, values.id);

                        try {
                          await DataStore.save(
                            Sale.copyOf(original!, (updated) => {
                              updated.title = values.title;
                              updated.content = values.content;
                              updated.buttonText = values.buttonText;
                            })
                          );
                        } catch (error) {
                          console.log(error);
